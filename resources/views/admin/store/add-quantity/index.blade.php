@extends('admin.layouts.main',[
								'page_header'		=> 'طلبات اضافة كمية معلقة',
            ...
            @if(!empty($records) && count($records)>0)
                <div class="table-responsive">
                    <table class="table table-bordered">
                        ...
                        <th class="text-center"> الكمية المطلوب اضافتها</th>
                        ...
                        @foreach($records as $record)
                            <tr id="removable{{$record->id}}">
                                ...
                                <td class="text-center">{{optional($record->product)->name}}</td>
                                <td class= "text-center">{{optional($record->supplier)->name}}</td>
                                <td class= "text-center">{{optional($record->product)->code}}</td>
                                <td class= "text-center">{{optional($record)->created_at}}</td>
                                <td class= "text-center">{{optional($record)->quantity}}</td>
                                <td class="text-center">
                                    <a href="{{route('store.add-quantity.show' , $record->id )}}"class="btn btn-primary pd-5"> عرض التفاصيل </a>
                                </td>
