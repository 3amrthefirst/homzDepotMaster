        ...
        <div class="ibox-content">
            <div class="table-responsive">
                ...
                <table class="table m-b-xs">
                    ...
                    <tr>
                        <td>
                            {{__('المقاسات')}} : <strong>{{$data->size}}</strong>
                        </td>
                        <td>
                            {{__('الخامات')}} : <strong>{{$data->material}}</strong>
                    ...
                    <tr>
                        <td>
                            {{__('الكمية المباعة')}} : <strong>{{$data->saledQuantity ?? "0"}}</strong>
                        </td>
                        <td>
                            {{__('الكمية المضافة')}} : <strong>{{array_sum($data->addQ()->pluck('quantity')->toArray())}}</strong>
                        </td>
                    </tr>
                    <tr>
                        <td>
                    ...
                    <tr>
                        <td>
                            {{__('السعر بعد تفعيل قيمة الخصم')}} : <strong>{{$data->ProductPriceAfterDiscountValue}}</strong>
                        </td>
                        <td>
                    ...
                    <tr>
                        ...
                        <td>
                            {{__('الكمية المسحوبة')}} : <strong>{{array_sum($data->pullQ()->pluck('quantity')->toArray())}}</strong>

                        </td>
                    </tr>
                    <tr>
                        <td>
                            تفعيل قيمه الخصم: {!! \App\MyHelper\Helper::toggleBooleanView($data , route('products.accepted.toggleBoolean',[$data->id,"discountValueStatus"]),'discountValueStatus') !!}
                         </td>
                         <td>
                            تفعيل نسبه الخصم: {!! \App\MyHelper\Helper::toggleBooleanView($data , route('products.accepted.toggleBoolean',[$data->id,"discountPercentStatus"]),'discountPercentStatus') !!}
                         </td>
                    </tr>
                    <tr>
                        <td>
